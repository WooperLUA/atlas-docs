...
export const ApiView = () => Layout(
        Section({},
            ...
            Div({className: 'api-item'},
                Code({textContent: 'createFlow<T>(name: string, initialState: T): T'}),
                P({textContent: 'Initializes a global state proxy in the registry.'}),
            ),
            Div({className: 'api-item'},
                Code({textContent: 'getFlow<T>(name: string): T'}),
                P({textContent: 'Retrieves a previously registered global state.'}),
            ),
            Div({className: 'api-item'},
                Code({textContent: 'getRefs<T>(proxy: T): { [K in keyof T]: () => T[K]'}),
                P({textContent: 'Converts state properties into reactive getter functions.'}),
            ),
